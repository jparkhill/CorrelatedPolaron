...
class AllPH: 
	def __init__(self):
		...
		if (Params.Adiabatic == 2 or Params.Adiabatic == 1) :
			self.RenormalizeIntegrals = False 
	...
	def InitAlgebra(self): 
		...
		if (os.path.isfile("./Terms/CIS") and os.path.isfile("./Terms/phphPterm")):
			...
			if (self.ForceAntiHermitian): 
				print "Adding Conjugate Terms."
				self.CISTerms.AddConjugateTerms()
				self.PTerms.AddConjugateTerms()
			...
			Tmp.QPAnnPart()
			Tmp.Add(FermiRho)
			self.VectorShape = Tmp.clone()
			print "Using Vector Shape: "
...
#			print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	...
	def DipoleMoment(self,AVector, Time=0.0): 
		...
		Mu = 0.0
		if (Params.Adiabatic == 0 and self.PTerms[0].BosInf != None) : 
			DressedMu = Integrals["mu_ph"].copy()
			...
			for i in range(DressedMu.shape[0]):
				for j in range(DressedMu.shape[1]):
					...
					for s in range(smax): 
						...
						BathSum += (TildeSum1)*(TildeSum2)*Css[s]
						wSum -= Coths[s]*(TildeSum1*TildeSum1 + TildeSum2*TildeSum2)/2.0
					try:
						DressedMu[i][j] *= numpy.exp(wSum+BathSum)
...
# Oh... Because the dipole moment should be real. Of course. 
		...
		Mu += numpy.tensordot(AVector["r1_hp"],DressedMu,axes=([0,1],[1,0]))		
		if (self.SecondOrderDipole): 
			for i in range(no):
				for j in range(no):
					for a in range(nv):
						for b in range(nv):	
	...
	def BathCorrelationMeasures(self): 
		...
		for Term in self.PTerms: 
			if (Term.WatchBCIndex == None): 
				Term.WatchBCIndex = numpy.unravel_index(Term.OldIntegrand.argmax(), Term.OldIntegrand.shape)
				integral += Term.CurrentIntegral[Term.WatchBCIndex]
				integrand += Term.OldIntegrand[Term.WatchBCIndex]
			else: 
				integral += Term.CurrentIntegral[Term.WatchBCIndex]
				integrand += Term.OldIntegrand[Term.WatchBCIndex]
	...
	def CISWeights(self,AVector): 
		if Params.BeginWithStatesOfEnergy == None: 
			...
			for i in rlen(self.CISEnergies): 
				if (abs(self.CISEnergies[i] - Params.BeginWithStatesOfEnergy) < 1./27.2113): 
...
#		print "Matrix: ", ToDiag
	...
	#		Bs.PlotCorrelationFunction()
		...
		if (Params.Adiabatic == 4):
			...
			print "DEBUG"
#			print "NonPertMtrx", self.NonPertMtrx
#			print "MarkovMatrix", self.MarkovMatrix
			Tmp = self.MarkovMatrix + self.NonPertMtrx			
			Tmp.resize(dim,dim)
			w3,v3 = numpy.linalg.eig(Tmp) 
			print "Energies of DEBUG (eV): ", (w3*EvPerAu)/complex(0.0,-1.0)
		...
		if (Params.BeginWithStatesOfEnergy != None): 
			...
			for i in rlen(Ens): 
				if (abs(Ens[i] - Params.BeginWithStatesOfEnergy) < 1./27.2113): 
					self.V0.Add(Sts[i])
...
#				Sts[s].Print()
			...
			print "SumSq: ", SumSq
		self.V0["r1_hp"] += numpy.transpose(self.V0["r1_ph"])
		self.V0.MultiplyScalar(1.0/numpy.sqrt(self.V0.InnerProduct(self.V0)))			
...
#		numpy.savetxt('./Output/MarkovMatrix', OverallMtrx ,fmt='%.18e')	
#		numpy.savetxt('./Output/MarkovMatrix', OverallMtrx ,fmt='%.18e')	
