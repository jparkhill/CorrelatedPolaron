...
class TightBindingTCL:
	def __init__(self):	
		...
		self.rho = np.zeros(shape=(7,7)) # This will be our density matrix
		self.rho[0,0] = 1 # and to start we set one of the values to one and leave the rest equal to zero.

		self.rho[1,1] = 0.5 # Fake values below just for testing purposes
		self.rho[2,2] = 0.74
		self.rho[3,3] = 1.39
		self.rho[4,4] = 2.08
		self.rho[5,5] = 0.0054
	...
	def numintJ(self, index, t0): # Here we will numerically integrate our J. Index denotes which site we are on.
		...
		# Testing the time it takes to do the for loop:
		#start = time.clock()
		#for i in xrange(10000): 
       	#		for ii in range(len(self.Wpts[index])):
		#			vals1.append(self.myterm(self.Wpts[index][ii], beta, t0) * self.Jinterp[index](self.Wpts[index][ii]))
		#print (time.clock() - start)/10000
		# result = 0.0047751086 s

		# Testing the time it takes to do the vectorized version:
		#start = time.clock()
		#for i in xrange(10000): 
       	#		vecMyTerm = np.vectorize(self.myterm)
		#	vals2 = vecMyTerm(w = self.Wpts[index], beta = beta, t0 = t0) * self.Jinterp[index](self.Wpts[index])	
		#print (time.clock() - start)/10000
		# result = 0.0019278011 s

		# Therefore, we use the vectorized version:
	...
	# Next steps:
	# 1. Find Thomas's RK4,5 routine to propagate d rho / d t = [HI,rho] + TCL(rho)
	# 2. Put all of this stuff back into Propagatable_TightTCL_7site.py. I need the machinery from the rest of the code
	# that prepares the initial state, applies the dipole operator at each step, and computes the absoption spectrum. There
	# is probably more machinery I need that I don't even know about / understand. 

	def test(self, t0):
		return self.TCL(self.rho, t0)



print TightBindingTCL().test(100)
